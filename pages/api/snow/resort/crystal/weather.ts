import { TerrainStatus, WeatherStatus } from "@/app/snow/SnowAPI";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!response.ok) {
    res.status(500).json({ error: `Error fetching Weather Status data for Crystal, WA from ${CRYSTAL_STATUS_URL}: ${response.status} ${response.statusText}`})
    return
  }

  const data = await response.json()
  const status: WeatherStatus = {
    tempCurrent: parseInt(data.CurrentConditions.MidMountain.TemperatureF),
    ...
    snowLastWeek: parseInt(data.SnowReport.MidMountainArea.Last7DaysIn),
